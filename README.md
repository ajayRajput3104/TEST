...
**NameRoll Number**\[Student Name 1\]\[Roll No 1\]\[Student Name 2\]\[Roll No 2\]\[Student Name 3\]\[Roll No 3\]\[Student Name 4\]\[Roll No 4\]\[Student Name 5\]\[Roll No 5\]
...
### A. Environment Logic: warehouse\_env.py
...
Plain textANTLR4BashCC#CSSCoffeeScriptCMakeDartDjangoDockerEJSErlangGitGoGraphQLGroovyHTMLJavaJavaScriptJSONJSXKotlinLaTeXLessLuaMakefileMarkdownMATLABMarkupObjective-CPerlPHPPowerShell.propertiesProtocol BuffersPythonRRubySass (Sass)Sass (Scss)SchemeSQLShellSwiftSVGTSXTypeScriptWebAssemblyYAMLXML`   # From warehouse_env.py  def _get_obs(self):      s = self.grid_size      return np.array([          self.robot_pos[0]/s, self.robot_pos[1]/s,  # Robot Coordinates          self.box_pos[0]/s, self.box_pos[1]/s,      # Box Coordinates          self.target_pos[0]/s, self.target_pos[1]/s,# Target Coordinates          1.0 if self.has_box else 0.0,              # State Flag          self.battery                               # Critical Resource      ], dtype=np.float32)   `
...
### B. The Algorithm: ppo\_scratch.py
...
### C. Testing Suite: stress\_test\_analytics.py
...
**Graph 1: Episode Reward (rollout/ep\_rew\_mean)**
...
**Graph 2: Episode Length (rollout/ep\_len\_mean)**
...
The model warehouse\_strict\_agent was subjected to 1,000 randomized test episodes.
...
*   **Output:** Generates warehouse\_strict\_agent.zip and TensorBoard logs in ./logs/strict/.
...
To create a video file (final\_presentation\_video.gif) showing the agent in action:
